...
def populate_dataset(
    ...
    for ep_idx in tqdm.tqdm(episodes):
        ...
        # ==================================================================================
        # Action Amplification Strategy
        # Logic: NewAction = CurrentState + (OriginalAction - CurrentState) * AMPLIFY_FACTOR
        # ==================================================================================
        AMPLIFY_FACTOR = 1.8
        
        # Based on file context (load_raw_episode_data), Arm Joints are at indices 12:26 (14 dimensions)
        # in the raw full-body data (0-11 are legs). We amplify only the arm joints.
        ARM_START, ARM_END = 12, 26
        
        # Safety check: Ensure state and action lengths match
        if state.shape[0] == action.shape[0]:
            # Amplify main action array (Arm indices only)
            action[:, ARM_START:ARM_END] = state[:, ARM_START:ARM_END] + \
                                           (action[:, ARM_START:ARM_END] - state[:, ARM_START:ARM_END]) * AMPLIFY_FACTOR
            
            # Apply to action_kuavo_arm_traj to maintain consistency
            if action_kuavo_arm_traj is not None and action_kuavo_arm_traj.size > 0:
                # action_kuavo_arm_traj corresponds to the arm slice dimensions (14 dims)
                if action_kuavo_arm_traj.shape[0] == state.shape[0]:
                     action_kuavo_arm_traj[:] = state[:, ARM_START:ARM_END] + \
                                                (action_kuavo_arm_traj - state[:, ARM_START:ARM_END]) * AMPLIFY_FACTOR
        # ==================================================================================
