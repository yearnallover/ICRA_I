...
# 设定加速因子，比如 1.5 或 2.0
DATA_SPEEDUP_FACTOR = 1.6  # 目标：30s -> ~18.75s
...
def populate_dataset(
    ...
    for ep_idx in tqdm.tqdm(episodes):
        ...
        # ==================== 【开始】插入加速代码 ====================
        # 设置加速倍率：
        SPEED_FACTOR = 1.5  # 目标加速倍率

        if SPEED_FACTOR > 1.0:
            n_frames = len(state)
            # 只有当帧数足够时才处理，避免极短数据报错
            if n_frames > 0:
                # 1. 计算保留的帧索引
                keep_indices = np.linspace(0, n_frames - 1, int(n_frames / SPEED_FACTOR)).round().astype(int)
                
                # 2. 基础数据切片 (这些通常不会为空)
                state = state[keep_indices]
                action = action[keep_indices]
                
                # 3. 【关键修正】对可能为空的硬件数据，先检查长度再切片
                if len(claw_state) > 0: claw_state = claw_state[keep_indices]
                if len(claw_action) > 0: claw_action = claw_action[keep_indices]
                
                if len(qiangnao_state) > 0: qiangnao_state = qiangnao_state[keep_indices]
                if len(qiangnao_action) > 0: qiangnao_action = qiangnao_action[keep_indices]
                
                if len(rq2f85_state) > 0: rq2f85_state = rq2f85_state[keep_indices]
                if len(rq2f85_action) > 0: rq2f85_action = rq2f85_action[keep_indices]
                
                if len(cmd_pos_world_action) > 0: cmd_pos_world_action = cmd_pos_world_action[keep_indices]

                # 处理轨迹数据 (保留 alt 的处理，防止解包后的数据长度不一致)
                if len(action_kuavo_arm_traj) > 0:
                    action_kuavo_arm_traj = action_kuavo_arm_traj[keep_indices]

                # 4. 可选数据切片
                if velocity is not None: velocity = velocity[keep_indices]
                if effort is not None: effort = effort[keep_indices]

                # 5. 图像切片 (必须做)
                for cam_name in imgs_per_cam.keys():
                    if len(imgs_per_cam[cam_name]) > 0:
                        imgs_per_cam[cam_name] = imgs_per_cam[cam_name][keep_indices]
                
                print(f"  >>> [加速生效] {n_frames} -> {len(state)} frames (倍率: {SPEED_FACTOR})")
        # ==========================================================
