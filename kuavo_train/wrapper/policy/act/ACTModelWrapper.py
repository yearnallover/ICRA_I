...
class DINOv3AsFeatureMap(nn.Module):
    """
    Wrap a transformers DINOv3 ViT model to return {"feature_map": (B, D, H//P, W//P)}.
    Handles CLS + register tokens dropping.
    """
    def __init__(self, dinov3_vit_model: nn.Module, patch_size: int = 16):
        super().__init__()
        self.m = dinov3_vit_model
        self.patch_size = patch_size
        self.out_channels = int(getattr(self.m.config, "hidden_size"))
    ...
    def forward(self, x: torch.Tensor):
        ...
        if patch_tokens.shape[1] != Hp * Wp:
            raise RuntimeError(f"Patch tokens={patch_tokens.shape[1]} != Hp*Wp={Hp*Wp}")

        feat_map = patch_tokens.transpose(1, 2).reshape(B, patch_tokens.shape[-1], Hp, Wp)  # (B, D, Hp, Wp)
        return {"feature_map": feat_map}


class Depth1ChTo3ChWrapper(nn.Module):
    """
    Make a 1-channel depth tensor compatible with a 3-channel ViT/DINO backbone by repeating channels.
    Returns same dict output as the wrapped backbone.
    """
    def __init__(self, backbone_3ch: nn.Module):
        super().__init__()
        self.backbone_3ch = backbone_3ch
        # expose out_channels for downstream proj init
        self.out_channels = getattr(backbone_3ch, "out_channels", None)

    def forward(self, x: torch.Tensor):
        # x: (B, 1, H, W) -> (B, 3, H, W)
        assert x.dim() == 4 and x.size(1) == 1, f"Expected (B,1,H,W), got {tuple(x.shape)}"
        x3 = x.repeat(1, 3, 1, 1)
        return self.backbone_3ch(x3)
...
class CustomACTModelWrapper(ACT):
    def __init__(self, config: CustomACTConfigWrapper):
        ...
        if self.config.use_depth and self.config.depth_features:
            dinov3_depth = DINOv3ViTModel.from_pretrained(
                "/root/kuavo_data_challenge/models/facebook/dinov3-vits16plus-pretrain-lvd1689m",  # 本地路径（你现在不能连HF）
                local_files_only=True,
            )
            depth_backbone_3ch = DINOv3AsFeatureMap(dinov3_depth, patch_size=16)
            self.depth_backbone = Depth1ChTo3ChWrapper(depth_backbone_3ch)

            self.encoder_depth_feat_input_proj = nn.Conv2d(
                self.depth_backbone.out_channels, config.dim_model, kernel_size=1
            )
