# 229 - 231 是官方原码, 234-251是我加的暴力提速补丁，这两部分代码二选一即可
# 我留着这个文件主要是因为234-251的暴力提速补丁很简洁，方便后面恢复
...
def run_single_episode(config, policy, preprocessor, postprocessor, episode, output_directory):
    ...
    while not done:
        ...
        # ==========================================
        # 🚀 暴力提速补丁 (Magic Speedup Patch)
        # ==========================================
        # 设置倍速因子 (支持整数，如 2, 3)
        # 2 表示 2倍速 (14秒完成)，3 表示 3倍速 (9秒完成)
        SPEED_FACTOR = 2
        
        with torch.inference_mode():
            # 先“空转”消耗掉 (SPEED_FACTOR - 1) 步
            # 作用：让模型内部的计数器快进，跳过中间的帧
            for _ in range(SPEED_FACTOR - 1):
                # 注意：这里传入 observation 只是为了满足接口，
                # 在 n_action_steps=100 的 Open-Loop 阶段，模型其实会忽略这个输入
                policy.select_action(observation)
            
            # 拿到这一帧真正要执行的动作 (即第 N 步的动作)
            action = policy.select_action(observation)
        # ==========================================
