# 这个版本实现了非整数跳帧采样和动作平滑滤波
# 使用的时候注意修改config.json中的n_action_steps参数为合适的值以及temporal_ensemble_coeff参数为null
...
def run_single_episode(config, policy, preprocessor, postprocessor, episode, output_directory):
    ...
    # Speedup and Smoothing Parameters
    SPEED_FACTOR = 1.35           # Speedup factor (supports decimal, e.g., 1.5x)
    # Define weights for smoothing [oldest, ..., newest]
    # SMOOTH_WEIGHTS = [0.0078, 0.0156, 0.0312, 0.0625, 0.125, 0.25, 0.5079]
    SMOOTH_WEIGHTS = [1]
    SMOOTH_WINDOW = len(SMOOTH_WEIGHTS)
    
    step_accumulator = 0.0
    action_history = []          # List to store recent actions for averaging
    ...
    while not done:
        ...
        # --- Speedup Logic: Skip steps based on SPEED_FACTOR ---
        step_accumulator += SPEED_FACTOR
        num_steps_to_consume = int(step_accumulator)
        step_accumulator -= num_steps_to_consume
        
        # Consume actions from policy and update history with ALL predictions
        # This ensures smoothing is done on the predicted trajectory, not just the sampled points
        with torch.inference_mode():
            for _ in range(num_steps_to_consume):
                raw_action = policy.select_action(observation)
                
                # Post-process and add to history immediately
                action_tensor = postprocessor(raw_action)
                current_numpy_action = action_tensor.squeeze(0).cpu().numpy()
                
                action_history.append(current_numpy_action)
                if len(action_history) > SMOOTH_WINDOW:
                    action_history.pop(0)

        # Calculate final action based on updated history
        if len(action_history) > 0:
            # Weighted average of the history buffer
            # Normalize weights if history is not full (e.g. at start)
            current_weights = SMOOTH_WEIGHTS[-len(action_history):]
            current_weights = np.array(current_weights) / np.sum(current_weights)
            numpy_action = np.average(action_history, axis=0, weights=current_weights)
        else:
            # Should not happen on first step if SPEED_FACTOR >= 1
            log_model.warning("No action generated and no history! Using zero action.")
            numpy_action = np.zeros(env.action_space.shape)
