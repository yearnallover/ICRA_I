...
class DropAssist:
    ...
    """
    def __init__(self, duration=10, wrist_pitch_target=1.57, z_lift_offset=0.5):
        self.active_arms = set()
        self.counter = 0
        self.duration = duration
        self.wrist_pitch_target = wrist_pitch_target
        self.z_lift_offset = z_lift_offset # Radian offset for shoulder/elbow to simulate Z lift
    ...
    def process_action(self, action, which_arm='both'):
        ...
        if self.counter > 0:
            ...
            # Apply Logic
            # 1. Wrist Pitch LERP
            # Make it fast! fully rotate down in first 33%
            blend = min(t * 3.0, 1.0)
            
            # 2. Z-Axis Lift (Approximate with Shoulder Pitch)
            # Lift Faster: Reach full height in first 25% of sequence
            # (4.0 > 3.0 ensuring lift leads slightly)
            lift_progress = min(t * 4.0, 1.0)
            lift = self.z_lift_offset * lift_progress
            
            if 'left' in self.active_arms and left_wrist_idx is not None and len(action) > left_wrist_idx:
                # LERP
                # We target WRIST PITCH (Index 5 for Left) instead of Roll (Index 6)
                pitch_idx = left_wrist_idx - 1
                # Left Arm Pitch is inverted relative to Right Arm. Use NEGATIVE target.
                action[pitch_idx] = action[pitch_idx] * (1-blend) + (-self.wrist_pitch_target) * blend
                action[left_shoulder_idx] -= lift # Assuming joint 0 is shoulder pitch and negative is UP
            
            if 'right' in self.active_arms and right_wrist_idx is not None and len(action) > right_wrist_idx:
                pitch_idx = right_wrist_idx - 1
                action[pitch_idx] = action[pitch_idx] * (1-blend) + self.wrist_pitch_target * blend
                action[right_shoulder_idx] -= lift
