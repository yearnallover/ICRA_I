...
import concurrent.futures  # 新增：异步线程池
...
def run_single_episode(config, policy, preprocessor, postprocessor, episode, output_directory):
    ...
    # ========== 新增：初始化异步线程池 ==========
    # 线程数建议等于摄像头数量（IO密集型任务，线程数过多无意义）
    executor = concurrent.futures.ThreadPoolExecutor(max_workers=len(cam_keys))
    # 存储所有异步保存任务的future对象，用于后续等待完成
    save_futures = []
    ...
    while not done:
        ...
        for k in cam_keys:
            ...
            # 1. 先在主线程处理图片数据（转CPU、格式转换），避免线程间数据竞争
            img = (observation[k].squeeze(0).cpu().numpy().transpose(1, 2, 0) * 255).astype(np.uint8)
            if img.shape[-1] == 1:
                img = img.squeeze(-1)
            # 2. 提交异步保存任务（非阻塞）
            future = executor.submit(
                imageio.imwrite,  # 要执行的函数
                str(frame_path),  # 参数1：文件路径
                img               # 参数2：图片数据
            )
    ...
    # ========== 新增：等待所有异步保存任务完成 ==========
    # 确保所有图片都保存完毕后，再进行视频合成
    log_model.info(f"Waiting for all image save tasks to finish (total: {len(save_futures)})")
    concurrent.futures.wait(save_futures)  # 等待所有任务完成
    # 检查异步任务是否有异常（可选，用于排查错误）
    for future in save_futures:
        try:
            future.result()  # 获取任务结果，若有异常会在此处抛出
        except Exception as e:
            log_model.error(f"Failed to save image in async task: {e}")
    # 关闭线程池
